...
class TimeSession(BaseModel):
    ...
    end_time: Optional[datetime] = Field(None, description="Session end time (None for active sessions)")
...
class DailyStats(BaseModel):
    ...
    most_used_tags: List[str] = Field(default_factory=list, description="Most frequently used tags")
    longest_session: Optional[float] = Field(None, description="Longest session duration in seconds")
...
class ProjectStats(BaseModel):
    ...
    session_count: int = Field(0, description="Number of sessions")
    average_session: float = Field(0.0, description="Average session duration in seconds")
    tags: List[str] = Field(default_factory=list, description="Associated tags")
    first_session: Optional[datetime] = Field(None, description="First session start time")
    last_session: Optional[datetime] = Field(None, description="Last session start time")