...
class SessionRepository:
    ...
    def get_sessions_in_range(self, start_date: date, end_date: date) -> List[TimeSession]:
    ...
    def get_sessions_by_tag(self, tag: str) -> List[TimeSession]:
        ...
        return [self._row_to_session(row) for row in rows if tag.lower() in json.loads(row['tags'])]
    ...
    def get_daily_stats(self, target_date: date) -> DailyStats:
        ...
        most_used_tags = sorted(tag_counts.keys(), key=lambda x: tag_counts[x], reverse=True)[:5]
        
        return DailyStats(
            date=target_date.isoformat(),
            total_duration=total_duration,
            session_count=len([s for s in sessions if s.end_time]),  # Only completed sessions
